...
To start a node from scratch:
  ...
  n := raft.StartNode(c, []raft.Peer{{NodeId: 0x02}, {NodeId: 0x03}})
...
Note: An NodeId represents a unique node in a cluster for all time. A
given NodeId MUST be used only once even if the old node has been removed.
...
stale log entries:
	...
	method. It is stored with sender's NodeId and later forwarded to leader by
	rafthttp package.
	...
	higher than follower's, the follower updates its leaderID with the NodeId
	from the message.
